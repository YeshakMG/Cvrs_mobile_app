...
class ServiceDetailController extends GetxController {
  ...
  List<String> get stepDescriptions {
    if (service.status == RequestStatus.rejected) {
      ...
      if (statusLogs.isNotEmpty) {
        final sortedLogs = statusLogs.toList()..sort((a, b) => a.updatedAt.compareTo(b.updatedAt));
        final rejectionLog = sortedLogs.firstWhere(
          (log) => log.status.toLowerCase().contains('reject'),
          orElse: () => sortedLogs.last,
        );
        final rejectionFormattedDate = DateFormat('dd-MM-yyyy').format(rejectionLog.updatedAt);
        return [
          firstDescription,
          'Rejected: ${service.rejectionReason ?? rejectionLog.remark ?? "Unknown reason"} on $rejectionFormattedDate',
          'Unable to proceed due to rejection',
          'Application terminated',
        ];
      }
      return [
        firstDescription,
        'Rejected: ${service.rejectionReason ?? "Unknown reason"} on ${service.rejectionDate ?? "Unknown date"}',
        'Unable to proceed due to rejection',
        'Application terminated',
      ];
    }
  ...
  void onInit() {
    ...
    if (Get.arguments != null && Get.arguments is ResidentService) {
      service = Get.arguments as ResidentService;
      fetchServiceDetail();
    } else {
